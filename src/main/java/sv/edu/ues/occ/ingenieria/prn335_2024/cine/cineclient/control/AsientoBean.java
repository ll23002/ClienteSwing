...
public class AsientoBean {
    ...
    public List<Asiento> findRange(int first, int max) {
        try {
            Response respuesta = webTarget.path("asiento")
                    .queryParam("first", first)
                    .queryParam("max", max)
                    .request(MediaType.APPLICATION_JSON).get();
            if (respuesta.getStatus() == 200) {
                List<Asiento> listaRespuesta = respuesta.readEntity(new GenericType<List<Asiento>>() {
                });
